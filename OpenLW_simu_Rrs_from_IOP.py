...
def Rrs_Fluorescence (v_chl,abs_cdom_nap_443,coeff_x=[0.0992,0.40,0.078],wavelength=np.array(range(400,851,5)),phi_f=0.01,sDir_IOP='./IOPfiles',str_path_Ed='Ed_Hydrolight_CA.txt'):
    """
    given chl concentration v_chl, estimate the Rrs contribution from fluorescence in wavelength range
    According to Eq. 7.5 and Eq. 7.27 in : Gilerson, A. A. and Y. Huot (2017). Chapter 7 - Bio-optical Modeling of Sun-Induced Chlorophyll-a Fluorescence. Bio-optical Modeling and Remote Sensing of Inland Waters. D. R. Mishra, I. Ogashawara and A. A. Gitelson, Elsevier: 189-231.
        v_chl: the value list of chlorophyll-a[mg/m^3] ; Nx1 list 
    ...
    if isinstance(str_path_Ed, type(None)) :
        str_path_Ed='Ed_Hydrolight_CA.txt'
    if not os.path.exists(str_path_Ed): 
        if os.path.exists(os.path.join(sDir_IOP,str_path_Ed)): 
            str_path_Ed=os.path.join(sDir_IOP,str_path_Ed)
        else:
            print(f'failed to intepret water iop path: {str_path_Ed}, will exit')
            return None
    Ed=load_iop_file(str_path_Ed,skip_footer=1,wavelength=wavelength) 
    Ed_in_air=Ed[:,-1]  # columns: Ed_diffuse	Ed_direct	Ed_total(W/m^2 nm)   
    ...
    # a_phy=list()  #the absorption coefficient of phytoplankton at λ
    # #the flux absorbed by phytoplankton
    # E0_= 1  #(μmol . photon. m-2. s-1. nm-1) is the scalar irradiance (i.e., the excitation irradiance) just below the surface
    # K_lambda=0.2  #the attenuation of scalar irradiance
    # K_Lu =0.5 #the attenuation of upwelling irradiance, KLu(685), accounts for the attenuation of upwelling fluorescence radiance
    ...
    ##further add a 2nd peak according to Mobley: On 6 July 2017 at 18:07, Curtis Mobley <curtis.mobley@gmail.com> 
    Lf_distribute=lambda wv:  np.exp(-4 * np.log(2) * ((wv -685)/25)**2 ) + 0.3 * np.exp(-4 * np.log(2) * ((wv -730)/50)**2 )
    Lf=Lf_685*Lf_distribute(wavelength)
    Rrs_f=Lf/Ed_in_air[None,:]  #note the Lf here is fluorescence radiance just above the surface, so /Ed will get above water Rrs
